...
| Claude Code, Codex, Qwen Code, OpenCode, Kimi Code, Gemini, Cursor, Copilot, Muse Code, Devin | Available | Presets for live coding sessions; custom stdin-based CLIs are supported too. [Qwen Code](qwen-code.md) supports native ACP and resumable sessions. [OpenCode and Kimi](chinese-coding-clis.md) add Chinese-provider options for task execution; triage and read-only reports require another provider. Muse Code is macOS/Linux/WSL2 only — its installer exits `unsupported platform` on Windows. Devin (Cognition) installs on every OS from its own script and signs in through a browser (`devin auth login`) |
