// The Studio is the Assistant Game: the office drawn in 3D, every figure in it backed by a real
// task, message, fyi or Hub file, and every real move (send, hand off, unblock, follow up) scores.
// Jump into a space (1-5), deal with what is there, Esc back out. The Agent Floor is the Board's
// floor as it always was - one desk per agent that can run at once, an empty desk is capacity.
...
function useSound() {
  const [on, setOn] = useState(() => { try { return localStorage.getItem("taskuary.studioGame.sound") !== "0"; } catch { return true; } });
  ...
  const toggle = () => setOn((v) => { try { localStorage.setItem("taskuary.studioGame.sound", v ? "0" : "1"); } catch { /* */ } return !v; });
...
export default function StudioView({ onOpenTask, refresh = 0, active = true }) {
  ...
  useEffect(() => { if (active && refresh) { load(); loadWorld(); } }, [active, refresh, load, loadWorld]);
  useEffect(() => {
  ...
  return (
    <Box sx={{ position: "relative", width: "100%", height: "calc(100vh - 190px)", minHeight: 600, overflow: "hidden",
      ...
      <Box sx={{ position: "absolute", inset: 0 }}>
        <React.Suspense fallback={<Box sx={{ position: "absolute", inset: 0, display: "grid", placeItems: "center" }}><CircularProgress size={22} /></Box>}>
          <StudioScene seats={sceneSeats} selectedId={pick} onSelect={(id) => { setPick(id); go("floor"); }}
            ...
            inset={wide ? { left: 214, right: 404 } : { left: 0, right: 0 }} />
        </React.Suspense>
      </Box>

      {/* ── the HUD ─────────────────────────────────────────────────────────────────────────── */}
      <Box sx={{ ...glass, position: "absolute", zIndex: 8, top: 10, left: 12, right: 12, px: 1.5, py: 1,
        ...
        <Box sx={{ display: { xs: "none", lg: "flex" }, gap: 0.75 }}>
          {QUESTS.map((q) => {
      ...
      <Box sx={{ position: "absolute", zIndex: 7, left: 12, top: 92, display: "flex", flexDirection: "column", gap: 0.6, width: { xs: 54, md: 190 } }}>
      ...
      <Box sx={{ ...glass, position: "absolute", zIndex: 7, right: 12, top: 92, bottom: 12, width: { xs: "calc(100% - 90px)", sm: 390 },
        display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <Box sx={{ px: 1.75, pt: 1.4, pb: 1.1, borderBottom: `1px solid ${G.line}` }}>
          <Typography sx={{ fontSize: 10, letterSpacing: 1.4, color: G.gold, fontWeight: 800 }}>
            {focus === "all" ? "MISSION BRIEFING" : `SPACE ${zoneMeta(focus)?.hotkey} · ESC TO LEAVE`}
          </Typography>
          <Typography sx={{ fontSize: 18, fontWeight: 900 }}>{focus === "all" ? "Your office right now" : zoneMeta(focus)?.name}</Typography>
          <Typography sx={{ fontSize: 11.5, color: G.dim }}>{focus === "all" ? "The assistant's picks: who should take each thing, and the move that gets it there." : zoneMeta(focus)?.blurb}</Typography>
        </Box>
        <Box sx={{ overflowY: "auto", flex: 1, minHeight: 0, px: 1.25, py: 1.1 }}>
      ...
      {/* ── rewards ───────────────────────────────────────────────────────────────────────────── */}
      <Box sx={{ position: "absolute", zIndex: 9, top: 96, left: "50%", transform: "translateX(-50%)", display: "flex", flexDirection: "column",
      ...
      {trophies && (
        <Box sx={{ ...glass, position: "absolute", zIndex: 9, top: 92, left: { xs: 12, md: 214 }, width: 320, maxHeight: "calc(100% - 110px)", overflowY: "auto", p: 1.5 }}>
...
function MapButton({ on, onClick, k, name, n }) {
  return (
    <Box component="button" type="button" onClick={onClick} title={name}
      sx={{ ...glass, display: "flex", alignItems: "center", gap: 1, px: 1, py: 0.75, cursor: "pointer", textAlign: "left", borderRadius: "11px",
      ...
      <Typography noWrap sx={{ display: { xs: "none", md: "block" }, fontSize: 12.5, fontWeight: 800, flex: 1 }}>{name}</Typography>
...
function Briefing({ pile, agents, onGo }) {
  const top = pile.filter(needsYou).slice(0, 6);
...
function CoreSpace({ chat, onAsk, busy, pile, agents, picked, onGo }) {
  ...
  const board = pile.filter((i) => needsYou(i) || i.kind === "agent").slice(0, 8);
  return <>
    <Box sx={{ mb: 1 }}>
      {chat.map((m, n) => (
